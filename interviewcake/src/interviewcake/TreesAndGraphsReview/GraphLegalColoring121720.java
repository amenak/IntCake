...
public class GraphLegalColoring121720 {
    ...
    public static void colorGraph(GraphNode[] graph, String[] colors) {
        ...
        if(graph.length <= 2) throw new IllegalArgumentException("can not legally color graph with less than 2 nodes");
        
        //loop over graph node. create a set of notallowed colors
        //loop over graph neighbors and add to the set
        //loop over colors array and assign the first color thats available 
        
        
        for(GraphNode node : graph) { //O(V)
        	Set<String> colorsNotAllowed = new HashSet<String>();
        	
        	//loop check
        	if(node.getNeighbors().contains(node)) {
        			throw new IllegalArgumentException ("graph loop detected"); 
        	}
        	
        	for(GraphNode neighbor : node.getNeighbors()) { //O(E)
        		if(neighbor.hasColor()) { 
        			colorsNotAllowed.add(neighbor.getColor());
        		}
        	}
        	
        	for(String c : colors) { //O(D)
        		if(!colorsNotAllowed.contains(c)) {
        			node.setColor(c);
        		}
        	}
        }
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(GraphLegalColoring121720.class);
